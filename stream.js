...
function MurmurHash(algorithm, options) {
  ...
  if (!(this instanceof MurmurHash)) return new MurmurHash(algorithm, options);
  ...
  if (algorithm instanceof MurmurHash) {
    this._handle = new algorithm._handle.constructor(
      algorithm._handle,
      endianness
    );
  } else if (algorithm) {
    ...
    throw new TypeError(
      "Must give algorithm string, a serialized state or a MurmurHash instance"
    );
  }
...
MurmurHash.prototype._transform = function(chunk, encoding, callback) {
  if (chunk.length < 8192) {
    // this constant was chosen experimentally
